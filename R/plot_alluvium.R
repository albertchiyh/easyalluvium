...
#' @examples
#' \dontrun{
#'   dt_5 <- read.csv("essay_grades.csv")
#'   plot_alluvium(data = dt_5,
...
plot_alluvium <- function(data, grouping_vars, fill_color = "yellow", stratum_color = "black",
  ...
  # Check if all grouping variables are present in the data
  if (!all(grouping_vars %in% names(data))) {
    stop("One or more grouping variables are not in the data.")
  }

  # Group data and summarize frequency
  data_freq <- data |>
    group_by(across(all_of(grouping_vars))) |>
    summarize(Freq = n(), .groups = "drop") |>
    ungroup()

  # Dynamically map axes based on the number of grouping variables
  axis_mapping <- setNames(paste0("axis", seq_along(grouping_vars)), grouping_vars)

  # Determine fill aesthetic based on whether fill_color is a column or a static color
  if (fill_color %in% names(data_freq)) {
    fill_aes <- aes_string(fill = fill_color)
  } else {
    fill_aes <- aes(fill = I(fill_color))
  }

  # Create the plot
  plot <- ggplot(data = data_freq, aes(y = Freq)) +
    geom_alluvium(aes_string(!!!axis_mapping), color = stratum_color) +
    fill_aes +
    geom_stratum() +
